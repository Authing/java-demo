...
const HttpManager = {
  ...
  // 修改角色信息
  editRole:(params) => post(`role/edit`,params),
  // 删除角色信息
  deleteRole:(code) => get(`role/delete?code=${code}`),
  // 添加角色
  addRole:(params) => post(`role/add`,params),
  // 返回除了 superAdmin 之外的所有角色
  getAllRoleWithOutSuperAdmin: () => get(`role/getAllRoleWithOutSuperAdmin`),
